...
def main():
    ...
    st.title("深度研报AI助手")
    st.markdown("告别漫无目的的搜索，迎接直达问题核心的深度研报。")
    ...
    with col1:
        ...
        query = st.text_area(
            "请输入您要研究的问题",
            placeholder="例如：2025-2035年适老化产业发展前景",
            height=100
        )
    ...
    with col2:
        ...
        if 'agent' in st.session_state and hasattr(st.session_state.agent, 'state'):
            ...
            status_placeholder.info("尚未开始研究")
...
def execute_research(query: str, config: Config, status_placeholder):
    ...
    def update_status_display(agent):
        """更新状态信息显示"""
...
def display_results(agent: DeepSearchAgent, final_report: str):
    ...
    with col1:
        st.metric("总段落数", progress['total_paragraphs'])
    with col2:
        st.metric("已完成段落", progress['completed_paragraphs'])
    with col3:
        st.metric("完成度", f"{progress['progress_percentage']:.0f}%")
